...
type ConnectionState struct {
	...
	//
	// Added in Go 1.26 (replacing the former internal testingOnlyDidHRR field)
	// - this struct's field layout must stay byte-for-byte identical to the
	// real crypto/tls.ConnectionState, since unsafe.go casts a pointer
	// directly between the two (see UnsafeFromConnectionState/
	// UnsafeToConnectionState and unsafe.go's init() layout-equality check,
	// which panics at process startup the moment this drifts - it caught
	// exactly this field addition).
